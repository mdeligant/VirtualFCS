...
model TestStackWithOneStageCompressorControlOER
  ...
  Modelica.Blocks.Sources.Ramp pressure_ramp(duration = 20, height = 0.3e5, offset = 1.3e5, startTime = 50) annotation(
    Placement(transformation(origin = {30, 97}, extent = {{-10, -10}, {10, 10}})));
  ...
  Modelica.Fluid.Sources.Boundary_pT AirSource(nPorts = 1, redeclare package Medium = Cathode_Medium) annotation(
    Placement(transformation(origin = {144, 44}, extent = {{10, -10}, {-10, 10}})));
  ...
  Modelica.Electrical.Analog.Basic.Ground ground annotation(
    Placement(transformation(origin = {109, 2}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Electrical.Analog.Sources.ConstantVoltage constantVoltage(V = 600) annotation(
    Placement(transformation(origin = {65, 12}, extent = {{-10, 10}, {10, -10}}, rotation = -0)));
equation
  ...
  connect(pressure_ramp.y, pid.SP) annotation(
    Line(points = {{41, 97}, {53.5, 97}, {53.5, 95}, {64, 95}}, color = {0, 0, 127}));
...
connect(fuelCellStack.OER, pid1.PV) annotation(
  ...
  connect(electricalCentrifugalCompressor.Output, fuelCellStack.port_a_Air) annotation(
    Line(points = {{53, 44}, {32, 44}, {32, 16}, {10, 16}}, color = {0, 127, 255}));
  ...
  connect(AirSource.ports[1], electricalCentrifugalCompressor.Input) annotation(
    Line(points = {{134, 44}, {76, 44}}, color = {0, 127, 255}));
  annotation(
...
end TestStackWithOneStageCompressorControlOER;