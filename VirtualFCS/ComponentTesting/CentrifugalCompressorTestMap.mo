...
model CentrifugalCompressorTestMap "Model to test the electrical centrifugal compressor with speed and pressure control"
  ...
  Modelica.Blocks.Sources.Ramp ramp1(height = 0*Modelica.Constants.pi/30, duration = 50, offset = 1000*Modelica.Constants.pi/30, startTime = 100) annotation(
    Placement(transformation(origin = {-76, 33}, extent = {{-10, -10}, {10, 10}})));
  ...
  Fluid.ElectricalCentrifugalCompressor electricalCentrifugalCompressor(redeclare package Medium = Medium)  annotation(
    Placement(transformation(origin = {-17, -9}, extent = {{-17, -17}, {17, 17}})));
  ...
  Modelica.Blocks.Sources.Ramp ramp2(duration = 400, height = -1, offset = 1, startTime = 50) annotation(
...
equation
  connect(AirSource.ports[1], electricalCentrifugalCompressor.Input) annotation(
    Line(points = {{-65, -11}, {-50.5, -11}, {-50.5, -9}, {-36, -9}}, color = {0, 127, 255}));
  connect(ramp1.y, electricalCentrifugalCompressor.controlInterface) annotation(
    Line(points = {{-65, 33}, {-44, 33}, {-44, 1}, {-36, 1}}, color = {0, 0, 127}));
  ...
  connect(electricalCentrifugalCompressor.Output, pressure.port) annotation(
    Line(points = {{2, -9}, {10.7, -9}, {10.7, -10}, {16, -10}}, color = {0, 127, 255}));
  ...
  connect(ramp2.y, valveLinear.opening) annotation(
    Line(points = {{25, 38}, {64, 38}, {64, -2}}, color = {0, 0, 127}));
  annotation(
    experiment(StopTime = 500, Interval = 0.5, Tolerance = 1e-06, StartTime = 0),
    __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=initialization,NLSanalyticJacobian",
    __OpenModelica_simulationFlags(lv = "LOG_STDOUT,LOG_ASSERT,LOG_STATS", s = "dassl", variableFilter = ".*"));
end CentrifugalCompressorTestMap;